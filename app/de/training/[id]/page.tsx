...
export default function DeTrainingModulePage() {
  const t = getTranslations('de');
  const params = useParams();
  const moduleId = parseInt(params.id as string);
  const trainingModule = trainingModulesDe.find((m) => m.id === moduleId);
  ...
  useEffect(() => {
    const savedProgress = localStorage.getItem(`training-de-${moduleId}`);
    if (savedProgress) {
    ...
    return (
      <div className="min-h-screen bg-flix-grayscale-10 pb-24">
        ...
        <main className="max-w-lg mx-auto px-5 py-8">
          <p className="text-[14px] text-flix-grayscale-70 mb-4">{t.training.lessonNotFound}</p>
          <Link href="/de/training" className="text-[14px] font-medium text-flix-primary hover:text-flix-ui-primary transition-colors">
        ...
        <main className="max-w-lg mx-auto px-5 py-8">
          <div className="text-center py-12 animate-fade-in">
            ...
            <h1 className="text-2xl font-semibold text-flix-grayscale-100 mb-2 tracking-tight">
              {t.training.youCrushedIt}
            </h1>
            <p className="text-[15px] text-flix-grayscale-70 mb-8">
              {t.training.completed} &quot;{trainingModule.title}&quot;
            </p>
            <div className="space-y-3">
              <Link href="/de/training">
                <button
                  type="button"
              ...
              {moduleId < trainingModulesDe.length && (
                <Link href={`/de/training/${moduleId + 1}`}>
                  <button
                    type="button"
    ...
    <div className="min-h-screen bg-flix-grayscale-10 pb-24">
      ...
      <main className="max-w-lg mx-auto px-5 py-8">
        ...
        >
            ...
            >
              {isLastLesson ? t.training.completeLesson : t.training.continue}
            </button>
          </div>
